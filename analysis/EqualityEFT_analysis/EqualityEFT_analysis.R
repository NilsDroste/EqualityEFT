...
# run the event study
# enactment
event_study_reg1 <- ES(long_data=full_df %>% data.table::setDT(), outcomevar="lnMun",
...
ES_plot_levels(event_study_reg1, lower_event = -3, upper_event = 10) + ylab("Mean of the Outcome")

ES_plot_ATTs(event_study_reg1, lower_event = -3, upper_event = 10) + ylab("ATT Estimate (95% CI)")

ES_plot_ATTs(event_study_reg1, lower_event = -3, upper_event = 10, homogeneous_ATT = TRUE) +   ylab("ATT Estimate (95% CI)")

# run the event study
# legislation
event_study_reg2 <- ES(long_data=full_df %>% data.table::setDT(), outcomevar="lnMun",
                       unit_var="ID", cal_time_var="year",
...
ES_plot_levels(event_study_reg2, lower_event = -3, upper_event = 10) + ylab("Mean of the Outcome")

ES_plot_ATTs(event_study_reg2, lower_event = -3, upper_event = 10) + ylab("ATT Estimate (95% CI)")

ES_plot_ATTs(event_study_reg2, lower_event = -3, upper_event = 10, homogeneous_ATT = TRUE) +   ylab("ATT Estimate (95% CI)")
