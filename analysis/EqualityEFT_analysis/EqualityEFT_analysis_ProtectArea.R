...
load(file="C://Users//Nils//Box//papers//work_in_progress//Cooperman_etal_2021_EqualityEFT//repo//data//completed_sets//State_panel_data.RData")
...
# create full dataset
full_df_ext <- complete_df %>% mutate(lnMun=log(munPA+const), lnSta=log(staPA+const2), lnFed=log(fedPA), lnTot=log(totPA), lnAg=log(share_ag_value_added), lnExtrInd=log1p(share_extr_ind_value_added), lnTransInd = log(share_trans_ind_value_added), lnPop=log(popdens), year=as.integer(year), legislation=as.integer(legislation), enactment=as.integer(enactment))

# run the event study
# enactment
event_study_reg_EFTenact_ext <- ES(long_data=full_df_ext %>% data.table::setDT(), outcomevar="lnMun",
                               unit_var="state", cal_time_var="year",
                               onset_time_var="enactment", cluster_vars="state",
                               discrete_covars = c('arpa','ama', 'cer', 'caa', "mat", "pan", "pam"),
                               cont_covars = c('lnAg','lnExtrInd', 'lnTransInd', 'lnPop', 'lnSta', 'lnFed'),
                               never_treat_action = "keep",
                               # linearize_pretrends = TRUE,
                               anticipation=1,
                               omitted_event_time = -3,
...
ES_plot_levels(event_study_reg_EFTenact_ext, lower_event = -3, upper_event = 10) + ylab("Mean of the Outcome")

ES_plot_ATTs(event_study_reg_EFTenact_ext, lower_event = -3, upper_event = 10) + ylab("ATT Estimate (95% CI)")

ES_plot_ATTs(event_study_reg_EFTenact_ext, lower_event = -3, upper_event = 10, homogeneous_ATT = TRUE) +   ylab("ATT Estimate (95% CI)")
...
PE.results <- PanelEstimate(sets = PM.results_ext, data = full_df_ext %>% mutate(ID_int = as.integer(state %>% as.factor)) %>% as.data.frame(), confidence.level = 0.90)
